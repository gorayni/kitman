...
class SegmentedPlayer:
    ...
    @staticmethod
    def _to_segmented_players(seg_img: Dict, min_score: float, filtered_class_ids: List[int]):
    ...
    def load(segmentation_fpath: Path, min_score: float = 0.0, class_id_filters: List[int] = None):
        ...
        return [SegmentedPlayer._to_segmented_players(s, min_score, class_id_filters) for s in segmented_images]
